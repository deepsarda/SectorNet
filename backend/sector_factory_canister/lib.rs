...
thread_local! {
    ...
    static RATE_LIMIT_MAP: RefCell<StableBTreeMap<StorablePrincipal, u64, Memory>> = RefCell::new(
        StableBTreeMap::init(
            MEMORY_MANAGER.with(|m| m.borrow().get(RATE_LIMIT_MAP_MEMORY_ID))
        )
    );

    // State that needs manual saving on upgrade
    static OWNER: RefCell<Option<Principal>> = RefCell::new(None);
    static SECTOR_WASM: RefCell<Option<Vec<u8>>> = RefCell::new(None);
...
fn pre_upgrade() {
    let state = NonStableState {
        ...
        sector_wasm: SECTOR_WASM.with(|w| w.borrow().clone()),
...
fn post_upgrade() {
    ...
    SECTOR_WASM.with(|w| *w.borrow_mut() = state.sector_wasm);
    REGISTRY_CANISTER_ID.with(|id| *id.borrow_mut() = state.registry_canister_id);
    INVITE_CANISTER_ID.with(|id| *id.borrow_mut() = state.invite_canister_id);
}
...
#[update]
fn set_sector_wasm(wasm_module: Vec<u8>) -> Result<(), Error> {
    is_owner()?;
    SECTOR_WASM.with(|w| *w.borrow_mut() = Some(wasm_module));
    Ok(())
}
...
fn set_invite_canister(id: Principal) -> Result<(), Error> {
    is_owner()?;
    INVITE_CANISTER_ID.with(|inv_id| *inv_id.borrow_mut() = Some(id));
    Ok(())
}
...
async fn create_new_sector(config: SectorConfig) -> Result<Principal, Error> {
    ...
    let wasm_module = SECTOR_WASM.with(|w| w.borrow().clone())
        .ok_or_else(|| Error::ConfigError("Factory is not configured with a Wasm module yet.".to_string()))?;
